...
part 'comment_remote.repository.g.dart';

/// Repository pour la récupération distante des commentaires.
class CommentRemoteRepository {
  ...
  /// Récupère la liste des commentaires pour une intervention.
  ///
  /// [idIntervention] : ID de l'intervention pour laquelle récupérer les commentaires.
  Future<CommentListResponseDto> fetchCommentsByIntervention({
    required String idIntervention,
  }) async {
    final response = await baseRemoteRepository.get(
      Urls.urls.commentsByIntervention(idIntervention),
    );
    ...
    if (response is Map<String, dynamic>) {
      ...
      return CommentListResponseDto.fromJson(response);
    }

    throw RemoteException(
      remoteMessage: 'Réponse inattendue du serveur lors de la récupération des commentaires',
    );
  }
}

@Riverpod(keepAlive: true)
CommentRemoteRepository commentRemoteRepository(Ref ref) {
